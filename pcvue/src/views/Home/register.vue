                ...
                <div class="login_right">
                    <div class="login_block">
                        ...
                        <div class="login_input">
                            <div class="input_block"><input v-model="info.mobile" type="text" placeholder="手机号"></div>
                            <div class="input_block"><input v-model="info.password" type="password" placeholder="密码"></div>
                            <div class="input_block"><input v-model="info.repeatPassword" type="password" placeholder="确认密码"></div>
                            <div class="input_block"><input v-model="info.code" type="text" class="yzm" placeholder="短信验证码">
                                <input  v-show="show" class="send" type="button" @click="send_sms()" value="发送验证码">
                                <button  v-show="!show" class="send" >{{count}}s后重新发送</button>
                            </div>
                        </div>
...
export default {
    ...
    methods: {
        ...
        send_sms:function(){
            ...
            this.$post(this.$api.homeRegCaptcha,{mobile:this.info.mobile}).then(res=>{
                if(res.errno == 0){
        ...
        to_register:function(){
            if(this.$isEmpty(this.info.mobile) || this.$isEmpty(this.info.password) || this.$isEmpty(this.info.repeatPassword) || this.$isEmpty(this.info.code)){
                return this.$message.error('请先认真填写完整');
            }
            if(this.info.password != this.info.repeatPassword){
                return this.$message.error('两次密码不相同');
            }
            this.info.username = this.info.mobile;
