        ...
        <div class="goods_info_top width_center_1200">
            ...
            <div class="goods_info_top_left" >
                ...
                <div class="pic_zoom_list">
                    ...
                    <ul>
                        <li v-for="(v,k) in goods_info.gallery" :key="k" @click="click_silde_img(k)" :class="chose_img_pos==k?'border_color':''"><img :src="v" alt=""></li>
                    </ul>
            ...
            <div class="goods_info_top_right" >
                ...
                <div class="goods_info_group">
                    <div class="goods_info_price"><span>商城价：</span>￥{{goods_info.retailPrice||'0.00'}}</div>
                    <div class="goods_info_market_price"><span>市场价：</span><div class="overx_goods_info">￥{{goods_info.counterPrice||'0.00'}}</div></div>
                    <div class="goods_info_active"><span>优惠：</span><font class="noy" v-if="goods_info.goods_freight<=0 && goods_info.freight_id<=0">包邮</font>
<!--                        <font v-else-if="store_info.free_freight>0">满{{store_info.free_freight}}包邮</font>-->
                        <font class="noz" v-else>暂无优惠</font></div>
                ...
                <div class="goods_info_spec" v-show="!this.$isEmpty(goods_info.chose_attr)">
                    <div class="spec_list" v-for="(v,k) in goods_info.spec_list" :key="k">
                        <span>{{v.name}}：</span>
                        <ul>
                            <li :class="($isEmpty(val.is_chose) || val.is_chose==false)?'':'red'" v-for="(val,key) in v.valueList" :key="key" @click="attr_click(k,key)" >{{val.value}}</li>
                        </ul>
                    </div>
                </div>
                ...
                <el-tabs v-model="activeName" @tab-click="handleClick">
                    <el-tab-pane label="商品详情" name="first">
                        <div class="goods_attribute" >
                            <ul v-show="attributes.length>0">
                                <li v-for="(v,k) in attributes" :key="k"><span>{{v.attribute}}</span> {{v.value}}</li>
                            </ul>
                        </div>
                        <div class="after_sale_content" v-html="goods_info.detail"></div>
                    </el-tab-pane>
                        ...
                        <div class="goods_info_comment_list" v-if="total_data>0">
                            <ul>
                                <li v-for="(v,k) in comment_list" :key="k">
                                    <div class="comment_avatar"><el-image :src="v.userInfo.avatarUrl" lazy></el-image></div>
                                    <div class="comment_nickname">{{v.userInfo.nickname}}</div>
                                    <div class="comment_star"><font>评价得分：</font><div class="store_star_in"><el-rate disabled :value="parseFloat(v.star)"></el-rate></div><div>{{v.score}}分</div></div>
                                    <div class="comment_content">评价内容：<font color="#999">{{v.content}}</font></div>
                                    <div class="comment_images" v-if="v.picList.length>0">
                                        <div class="comment_image" v-for="(vo,key) in v.picList" :key="key"><el-image @click="push_comment_images(v.picList)" :preview-src-list="comment_image"  :src="vo" lazy></el-image></div>
                                    </div>
                                </li>
                            </ul>
...
export default {
    ...
    data() {
      return {
        ...
        //   isSkeleton:false, // false 骨架显示
          ...
          page_size:20,
          current_page:1,
          comment_type:0,//评论类型，如果是0则是商品评论，如果是1则是专题评论
          bfb:100,
          star:5.0,
          comment_num:{
              good:0,
    ...
    methods: {
        ...
        get_comment_list:function(){
            this.$get(this.$api.homeGetCommentList,{valueId:this.goods_id,type:this.comment_type,page:this.current_page,showType:0,limit:20}).then(res=>{
        ...
        pre_img:function(){
            if(this.chose_img_pos<=0){
                this.chose_img_pos = this.goods_images.length-1;
            }else{
                this.chose_img_pos -= 1;
            }
        },
        next_img:function(){
            if(this.chose_img_pos>=this.goods_images.length-1){
        ...
        // },
        goods_fav:function(){
            this.$post(this.$api.homeAddOrDeleteFav,{valueId:this.goods_id,type:0}).then(()=>{
        ...
        attr_click:function(e,index){
            this.goods_info.spec_list[e]['valueList'].forEach((res,key)=>{
                res.is_chose = false;
                if(key == index){
                    res.is_chose = true;
                }
            ...
            this.goods_info.spec_list.forEach(res=>{
                res['valueList'].forEach(listRes=>{
                    if(!this.$isEmpty(listRes.is_chose) && listRes.is_chose == true){
                        chose_spec.push(listRes.value);
                    }
                });
            });
            
            this.chose_spec = chose_spec;
            // window.console.log(this.chose_spec);
        ...
        get_spec_attr_data:function(){
             this.goods_info.chose_attr.forEach(res=>{
                let a = 0;
                this.chose_spec.forEach(specRes=>{
                    // let resInfo = res.spec_name.split(' ');
                    if(res.specifications.indexOf(specRes)>=0){
                        a += 1;
                    }
                });
        ...
        buy:function(){
            ...
            let str = this.goods_info.id+'|'
                        // +this.goods_info.store_info.id
                        +'|'+this.buy_num;
            if(!this.$isEmpty(this.chose_spec_info['id']) && this.chose_spec_info['id']>0){
                str += '|'+this.chose_spec_info['id'];
            }
            
            this.goods_info['goodsId'] = this.goods_id;
            this.goods_info['productId'] = this.chose_spec_info.id;
            this.goods_info['number'] = this.buy_num;
            this.$post(this.$api.fastAddCart,this.goods_info).then(res=>{
                // window.console.log(res);
                if(res.errno == 0){
                    // this.get_goods_info();
        ...
        add_cart:function(){
            ...
            // if(this.chose_spec_info.length>0){
            ...
            if(goods_list.length>0){
                goods_list.forEach(res=>{
                    if(goods_info.id == res.id){
                        ...
                        res.image = goods_info.picUrl;
                        res.id = goods_info.id;
                        res.goods_price = goods_info.retailPrice;
                        have_his = true;
                    }
                })
            }
        ...
        get_goods_info:function(){
            this.$get(this.$api.homeGetGoodsInfo,{id:this.goods_id}).then(res=>{
                // window.console.log(res.data);
                this.goods_info = res.data.info;
                this.goods_info.chose_attr = res.data.productList;
                this.goods_info.spec_list = res.data.specificationList;
                this.store_info = res.data.store_info;
                this.goods_images_thumb = res.data.info.gallery;
                this.goods_images = res.data.info.gallery;
                this.attributes = res.data.attribute;
                this.buy_num = 1; // 初始化购买数量
                this.chose_spec = [];
                this.chose_spec_info = {};
                this.is_spec_goods();
                this.issues = res.data.issue;
                this.goods_info.comment_num = res.data.comment.count;
                this.is_fav =  res.data.userHasCollect==0?false:true;
...
.after_sale_content /deep/ img{vertical-align: text-bottom;}
.goods_attribute{
    font-size: 14px;
    padding: 10px 0 10px;
    ul{
        li{
            ...
            float: left;
            margin-bottom:10px;
            span{font-size:14px;width: 200px; color: #666;}
        }
