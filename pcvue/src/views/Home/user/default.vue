        ...
        <div class="user_info_order_num_show">
            <ul>
                <li><router-link :to="{ name: 'user_order',params:{order_type:'NOPAY'}}">
                     <el-badge :value="order_num.unpaid" class="item" :hidden="order_num.unpaid==0">
                    <i class="icon iconfont">&#xe617;</i>
                    <span>待支付</span>
                    </el-badge>
                </router-link></li>
                <li><router-link :to="{ name: 'user_order',params:{order_type:'WAITSEND'}}">
                    <el-badge :value="order_num.unship" class="item" :hidden="order_num.unship==0">
                    <i class="icon iconfont">&#xe63b;</i>
                    <span>待发货</span>
                    </el-badge>
                </router-link></li>
                <li><router-link :to="{ name: 'user_order',params:{order_type:'WAITREC'}}">
                    <el-badge :value="order_num.unrecv" class="item" :hidden="order_num.unrecv==0">
                    <i class="icon iconfont">&#xe61d;</i>
                    <span>待收货</span>
                    </el-badge>
                </router-link></li>
                <li><router-link :to="{ name: 'user_order',params:{order_type:'WAITCOMMENT'}}">
                    <el-badge :value="order_num.uncomment" class="item" :hidden="order_num.uncomment==0">
                    <i class="icon iconfont">&#xe610;</i>
                    <span>待评论</span>
                    </el-badge>
                </router-link></li>
...
<!--                    <div class="user_index_store_title"><router-link :to="'/store/'+v[0].store_id"><img width="40px" height="40px" :src="v[0].store_logo">{{v[0].store_name}}<span>前往店铺</span></router-link></div>-->
                    <ul>
                        <li v-for="(vo,key) in order_list" :key="key">
                            <router-link :to="'/goods/info/'+vo.goodsList[0].id">
                                <div class="user_order_goods_thumb"><img width="40px" height="40px" :src="vo.goodsList[0].picUrl" alt=""></div>
                                <div class="user_order_goods_title">{{vo.orderStatusText}}</div>
                                <div class="user_order_goods_price">￥{{vo.actualPrice}}</div>
                                <div class="user_order_goods_num">订单号：{{vo.orderSn}}</div>
                            </router-link>
                        </li>
                    </ul>
                </div>
        ...
        <div class="user_index_myorder">
            <div class="user_index_myorder_title">
                近期收藏<span><router-link to="/user/fav">查看更多</router-link></span>
            </div>
            <div class="user_index_line"></div>
                ...
                <div class="user_index_store_list">
                    <ul>
                        <li v-for="(v,k) in fav_list" :key="k">
                            <router-link :to="'/goods/info/'+v.valueId">
                            <div class="user_order_goods_thumb"><img width="40px" height="40px" :src="v.picUrl" :alt="v.name"></div>
                            <div class="user_order_goods_title">{{v.name}}</div>
                            <div class="user_order_goods_price">￥{{v.retailPrice}}</div>
                            <div class="user_order_goods_btn"><router-link :to="'/goods/info/'+v.valueId">前往商品</router-link></div>
                            </router-link>
                        </li>
...
export default {
    ...
    methods: {
        get_default_info:function(){
            ...
            this.$get(this.$api.homeGetUserOrder,{}).then(res=>{
                this.order_list = res.data.list;
            });
            this.$get(this.$api.homeCollectList,{type:0}).then(res=>{
                this.fav_list = res.data.list;
            });
        },
