    ...
    <div class="create_order">
        ...
        <div class="address_chose width_center_1200">
            ...
            <div class="address_list" v-if="address_list.length>0">
                <ul>
                    <li :class="v.isDefault==true?'red':''" v-for="(v,k) in address_list" :key="k" @click="set_default(v.id)">
                        <div class="receive_name">{{v.name}} <span>({{v.tel}})</span></div>
                        <div class="area_info">{{v.province+' '+v.city+' '+v.county+' '+v.addressDetail}}</div>
                        <div :class="v.isDefault==true?'is_default_red':'is_default'"><i class="icon iconfont">&#xe611;</i></div>
                    </li>
                </ul>
            </div>
            ...
            <div class="order_list_title">
                <el-row>
                    <el-col :span="10"><div class="th_title">商品</div></el-col>
                    <el-col :span="4"><div class="th_title">规格</div></el-col>
                    ...
                    <ul>
                        <li v-for="(vo,key) in befor_order_list" :key="key">
                            <el-row>
                                <el-col :span="10"><div class="goods_info">
                                    <dl>
                                        <dt><el-image :src="vo.picUrl||'/pc/loading_pic_200.png'"></el-image></dt>
                                        <dd>{{vo.goodsName}}</dd>
                                    </dl>
                                </div></el-col>
                                <el-col :span="4"><div class="th_title">{{$isEmpty(vo.specifications)?' - ':vo.specifications.join(',')}}</div></el-col>
                                <el-col :span="4"><div class="th_title">￥{{vo.price}}</div></el-col>
                                <el-col :span="2"><div class="th_title" style="text-indent:30px;">x {{vo.number}}</div></el-col>
                                <el-col :span="2"><div class="th_title" style="text-indent:40px;"> - </div></el-col>
                                <el-col :span="2"><div class="x_price" style="text-indent:30px;">￥{{vo.price*vo.number}}</div></el-col>
                            </el-row>
                        </li>
                    </ul>
                </div>
            ...
            <div class="foot_create_order">
                ...
                <div class="money_pay" v-show="!$isEmpty(user_info.money)">
...
export default {
    ...
    methods: {
        ...
        set_default:function(id){
            ...
            // });
            setLocalStorage({AddressId:id});
            this.address_list.forEach(addressRes=>{
                if(addressRes.id==id)
                    addressRes.isDefault = true;
                else addressRes.isDefault = false;
            });
        },
        create_order:function(){
            ...
            this.$post(this.$api.homeCreateOrder,
                        {
                            addressId: AddressId,
                            cartId: CartId,
                            couponId: CouponId,
                    ...
                    if(res.errno != 0){
                        ...
                        // }else{
                        this.$message.success('订单创建成功，等待支付');
                        return this.$router.push('/order/chose_pay/'+res.data.orderId);//+'/'+this.$route.params.type+'/'+this.$route.params.info)
                        // }
        ...
        get_order_list:function(){
            ...
            //     this.befor_order_list = res.data.cartList;
            //     this.befor_order_list.forEach(goodsRes=>{
            //         // res.forEach(goodsRes=>{
            //             this.total_price += (goodsRes.price*goodsRes.number);
            //         // });
            //     });
            // });
            const {AddressId, CartId, CouponId, UserCouponId} = getLocalStorage('AddressId', 'CartId', 'CouponId', 'UserCouponId');

            this.$get(this.$api.homeCheckoutCart).then(res=>{
                // window.console.log(res);
                this.befor_order_list = res.data.checkedGoodsList;
                this.total_price = res.data.goodsTotalPrice;
                this.total_freight = res.data.freightPrice;
                setLocalStorage({AddressId: res.data.addressId, CartId: res.data.cartId, CouponId: res.data.couponId, UserCouponId: res.data.userCouponId});
