...
public class WxOrderService {
    ...
    public Object nativePay(Integer userId, String body, HttpServletRequest request) {
        ...
        try {
            WxPayUnifiedOrderRequest orderRequest = new WxPayUnifiedOrderRequest();
            orderRequest.setOutTradeNo(order.getOrderSn());
            orderRequest.setProductId(order.getId().toString());//productId 非空
     ...
     */
    @Transactional
    public Object aliPcPay(Integer userId, String body, HttpServletRequest request) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        Integer orderId = JacksonUtil.parseInteger(body, "orderId");
        ...
        for(LitemallOrderGoods goods: orderGoodsList){
            subject = subject+goods.getGoodsName()+" ";
            for(String spec: goods.getSpecifications()) {
                orderBody = orderBody + spec + " ";
            }
        }
        if (order == null) {
            return ResponseUtil.badArgumentValue();
...
//            orderRequest.setTotalFee(fee);
//            orderRequest.setSpbillCreateIp(IpUtil.getIpAddr(request));
            result = AlipayUtil.pay(order.getOrderSn(),order.getActualPrice()+"",subject.trim(),orderBody.trim(),null);
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//        logger.info(params);
        ...
        String trade_status,payId="",orderSn="",totalFee="";
        try {
        if (signVerified) {// 验证成功
            ...
            //支付宝交易号
            payId = new String(request.getParameter("trade_no").getBytes("ISO-8859-1"), "UTF-8");
            //商城订单号
            orderSn = new String(request.getParameter("out_trade_no").getBytes("ISO-8859-1"), "UTF-8");
            //
