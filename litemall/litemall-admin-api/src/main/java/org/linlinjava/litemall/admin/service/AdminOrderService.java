...
import org.linlinjava.litemall.core.util.AlipayUtil;
...
public class AdminOrderService {
    ...
    public Object refund(String body) {
        ...
        if(order.getPayType().equals("wxpay")) {
            ...
            String result = AlipayUtil.aliRefund(order.getOrderSn(),order.getPayId(),order.getActualPrice()+"","正常退款","");
            if(result.indexOf("\"code\":\"10000\"")<0)
                return ResponseUtil.fail(ORDER_REFUND_FAILED, "订单退款失败");
            order.setRefundType("支付宝退款接口");
        }
